  ...
  private _buildGeneralFormRoot(): HTMLElement {
    return el('section', { class: 'settings' },
      ...
      this.field('history retention (days)', this.retentionInput,
        'Only overlay pointers for sessions grok has already dropped are pruned. Lived-in TUI sessions, starred rows, and archived rows are never auto-removed. 0 disables cleanup. Default 30.'),
    ...
    try {
      ...
      if (modelsResp.status === 'fulfilled' && modelsResp.value) {
        ...
        if (Array.isArray(m)) models = m;
          ...
          const obj = m as { models?: unknown[]; items?: unknown[] };
          if (Array.isArray(obj.models)) models = obj.models;
          else if (Array.isArray(obj.items)) models = obj.items;
        }
      }
      this.models = models;
      this.swapModelField();
