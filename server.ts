...
import { AgentManager, SessionHeldError, type PublicAgent } from './lib/agent-manager.js';
import { sessionHeldPayload } from './lib/session-ownership.js';
import { deleteAgentWithOptionalTui, wantsDeleteTuiSession } from './lib/delete-agent.js';
...
const NO_LISTEN = process.env['GROK_REMOTE_NO_LISTEN'] === '1';

export let manager: AgentManager = null as unknown as AgentManager;
if (!NO_LISTEN) manager = new AgentManager();

export function setManagerForTests(m: AgentManager): void {
  manager = m;
}
...
export async function handleApi(req: IncomingMessage, res: ServerResponse, url: string, method: string): Promise<void> {
  ...
  if (url === '/api/agents' && method === 'POST') {
    try {
      ...
      const resumeSessionId = typeof body['resumeSessionId'] === 'string'
        ? body['resumeSessionId'].trim()
        : '';
      const cwd = typeof body['cwd'] === 'string'
        ? body['cwd']
        : (!body['cwd'] && defaults.defaultCwd ? defaults.defaultCwd : undefined);
      if (!resumeSessionId && !resolveStartCwd(cwd, defaults.defaultCwd)) {
        sendJson(res, 400, { ok: false, error: 'cwd required' });
        return;
      }
      const existing = resumeSessionId ? manager.getByIdOrSession(resumeSessionId) : null;
      const wasArchived = !!(existing && existing.archived);
      const rec = await manager.createFromPost({
      ...
      if (wasArchived) {
        try { setArchivedForAgent(rec.id, false); }
        catch { /* folder side-effect should never fail the create response */ }
      }
  ...
  if (route) {
    const rec = manager.getByIdOrSession(route.id);
    if (!rec) { sendJson(res, 404, { ok: false, error: 'agent not found' }); return; }
    const id = rec.id; // rebound overlay UUID — every manager.* below uses this
    const pub = manager.get(id);
    if (!pub) { sendJson(res, 404, { ok: false, error: 'agent not found' }); return; }
    const { suffix } = route;

    if (suffix === '' && method === 'GET') {
      sendJson(res, 200, manager.get(id));
      return;
    }
    if (suffix === '' && method === 'PATCH') {
      try {
        const body = (await readJsonBody(req) || {}) as Record<string, unknown>;
        await manager.update(id, body as never);
    ...
    if (suffix === '' && method === 'DELETE') {
      const raw = manager.getRaw(id);
      const pub = manager.get(id);
      const out = await deleteAgentWithOptionalTui({
        deleteTuiSession: wantsDeleteTuiSession(req.url),
        heldBy: pub?.heldBy ?? null,
        // ACP children are not in active_sessions.json — connected is the writer.
        hasLocalAcp: !!(raw && raw.client),
        localPid: raw?.client?.proc?.pid ?? null,
        sessionId: rec.lastSessionId || rec.client?.sessionId || null,
        cwd: rec.cwd,
        overlayId: id,
        kill: () => manager.kill(id),
      });
    ...
    if (suffix === '/prompt' && method === 'POST') {
      try {
        ...
        if (err instanceof SessionHeldError) {
          sendJson(res, 409, sessionHeldPayload(err));
          return;
        }
        const msg = err instanceof Error ? err.message : String(err);
        sendJson(res, 400, { ok: false, error: msg });
    ...
    if (suffix === '/model' && method === 'POST') {
      try {
        const body = (await readJsonBody(req) || {}) as { model?: unknown; reasoningEffort?: unknown };
        const out = await manager.switchModel(id, {
          model: typeof body.model === 'string' ? body.model : undefined,
        ...
        const msg = err instanceof Error ? err.message : String(err);
        const code = /wait until this turn|reconnect to switch|not ready|archived/i.test(msg) ? 409 : 400;
        sendJson(res, code, { ok: false, error: msg });
      }
      return;
    }
    ...
    if (suffix === '/connect' && method === 'POST') {
      try {
        ...
        sendJson(res, 202, manager.get(id));
      } catch (err) {
        if (err instanceof SessionHeldError) {
          sendJson(res, 409, sessionHeldPayload(err));
          return;
    ...
    if (suffix === '/history' && method === 'GET') {
      ...
      const sliced = resolveConversationHistory({
        agentId: id,
        sessionId: pub.sessionId || pub.lastSessionId,
        cwd: pub.cwd,
        all,
        turns,
      });
      res.writeHead(200, {
        ...
        'X-History-Source': sliced.source,
      });
      res.end(sliced.text);
      return;
    }
...
function handleStream(req: IncomingMessage, res: ServerResponse, id: string): void {
  ...
  // Browser close must only drop this SSE subscriber. Never cancel or
  // kill the grok agent — the turn keeps running on the server.
  let cleaned = false;
...
export async function dispatchRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  const url = (req.url || '').split('?')[0] || '/';
  if (url === '/api/system/sessions' && (req.method || 'GET') === 'GET') {
    // req.url still has q/limit/includeEmpty; `url` was stripped above.
    sendJson(res, 200, listJoinedSystemSessions(req.url || '/', manager.list()));
    return;
  }
