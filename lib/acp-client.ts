...
export class AcpClient extends EventEmitter {
  ...
  private async _handshake(resumeSessionId: string | null): Promise<void> {
    ...
    if (resumeSessionId && canLoad) {
      try {
        const loadedSession = await this.request('session/load', {
          sessionId: resumeSessionId,
          cwd: this.cwd,
          mcpServers: [],
        });
        this.sessionId = resumeSessionId;
        this._ingestSnapshot(loadedSession);
        loaded = true;
    ...
    if (!loaded) {
      ...
      this._ingestSnapshot(ns);
      this.emit('session_ready', { sessionId: this.sessionId, resumed: false });
    }
  }

  private _ingestSnapshot(result: unknown): void {
    const snap = ingestSessionSnapshot(result);
    if (snap.modelId) this.modelId = snap.modelId;
    if (snap.reasoningEffort) this.reasoningEffort = snap.reasoningEffort;
  }

  private async _refreshIdentity(): Promise<void> {
    if (!this.sessionId) return;
    try {
      const info = await this.request('_x.ai/session/info', { sessionId: this.sessionId });
      const model = ingestSessionInfo(info);
      if (model) this.modelId = model;
    } catch { /* optional extension */ }
  }
  ...
  async waitUntilReady(timeoutMs = 15000): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const onErr = (err: unknown): void => { cleanup(); reject(err); };
      const cleanup = (): void => {
        clearTimeout(timer);
        this.off('session_ready', onReady);
        this.off('error', onErr);
      };
      this.on('session_ready', onReady);
      this.on('error', onErr);
      if (this.sessionId) {
        cleanup();
        resolve();
  ...
  async setModel(modelId: string): Promise<void> {
    const id = trimText(modelId);
    if (!id) throw new Error('modelId required');
    if (!this.sessionId) throw new Error('session not ready');
    await this.request('session/set_model', { sessionId: this.sessionId, modelId: id });
    this.modelId = id;
  }

  async setMode(modeId: string): Promise<void> {
    const id = trimText(modeId);
    if (!id) throw new Error('modeId required');
    if (!this.sessionId) throw new Error('session not ready');
    await this.request('session/set_mode', { sessionId: this.sessionId, modeId: id });
